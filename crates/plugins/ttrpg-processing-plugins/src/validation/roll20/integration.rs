...
// TODO: Re-enable Roll20 validation integration once missing types are implemented
// Currently commented out due to unresolved imports for Roll20Parser, Roll20ValidationEngine,
// ValidationService, AssetService, LoggingService, and other missing types

/*
use crate::validation::core::ValidationPlugin;
use ttrpg_core::{
    error::{ConversionError, ConversionResult},
    plugin_framework::{
        interfaces::LoggingPlugin,
        types::{AssetReference, ConversionStats, ValidationResult as CoreValidationResult},
    },
    types::{Campaign, ValidationResult},
};

// Roll20Parser now in ttrpg-input-plugins crate
// Roll20Campaign structures moved to ttrpg-core types
...
impl ValidatedRoll20Parser {
    ...
    pub async fn parse_and_validate_file<P: AsRef<Path>>(
        &mut self,
        file_path: P,
    ) -> ConversionResult<CoreValidationResult> {
        ...
        let content = tokio::fs::read_to_string(&file_path).await.map_err(|e| {
            ConversionError::io_error(
                "ValidatedRoll20Parser",
                format!("Failed to read Roll20 file: {e}"),
            )
        })?;

        let roll20_data: serde_json::Value = serde_json::from_str(&content).map_err(|e| {
        ...
        let roll20_campaign: Roll20Campaign =
            serde_json::from_value(roll20_data.clone()).map_err(|e| {
                ConversionError::validation(
                    "ValidatedRoll20Parser",
        ...
        let validation_result = self
            .validation_engine
            .validate_roll20_conversion(&context)?;
        ...
        Ok(CoreValidationResult {
            is_valid: validation_result.is_valid,
            issues: validation_result
                .issues
                .into_iter()
                .map(|issue| ttrpg_core::plugin_framework::types::ValidationIssue {
                    severity: ttrpg_core::plugin_framework::types::IssueSeverity::Error,
                    entity_type: issue.category.to_string(),
                    entity_id: issue.context.entity_name.clone(),
                    field: None,
                    message: issue.message,
                    suggestion: issue.suggested_fix,
                })
                .collect(),
            stats: ttrpg_core::plugin_framework::types::ValidationStats {
                entities_validated: validation_result.performance_metrics.entities_validated,
                entities_with_issues: validation_result.stats.errors
                    + validation_result.stats.warnings,
                validation_time_ms: validation_result.performance_metrics.validation_duration_ms
                    as u64,
                cache_hit_ratio: 0.0,
            },
        })
    ...
    pub async fn parse_and_validate_json(
        ...
        // Parse JSON
        let roll20_data: serde_json::Value = serde_json::from_str(json_content).map_err(|e| {
            ConversionError::validation(
                "ValidatedRoll20Parser",
                format!("Invalid Roll20 JSON: {e}"),
            )
        ...
        // Validate
        let context = crate::validation::roll20::validation::Roll20ConversionContext {
            roll20_data: &roll20_data,
            converted_campaign: campaign.clone(),
            conversion_stats: self.conversion_stats.clone(),
            asset_references: self.discovered_assets.clone(),
        };
    ...
    async fn convert_with_asset_discovery(
        ...
        let mut campaign = Campaign {
            metadata: ttrpg_core::types::CampaignMetadata {
                name: "Test Campaign".to_string(),
                source_format: ttrpg_core::types::SourceFormat::Roll20,
                schema_version: None,
                created_at: None,
                stats: ttrpg_core::types::CampaignStats::default(),
            ...
            macros: vec![],
            folders: vec![],
            assets: ttrpg_core::types::AssetCollection::default(),
        };
    ...
    fn create_campaign_metadata(
        &self,
        campaign_info: &ttrpg_core::plugin_framework::registry::Roll20Campaign,
...
pub struct Roll20ValidationResult {
    ...
    pub validation_result: crate::validation::roll20::validation::Roll20ValidationResult,
...
impl Roll20ValidationResult {
    ...
    pub fn generate_report(&self) -> String {
        ...
        // Header
        report.push_str(&format!("=== Roll20 Conversion Validation Report ===\n\n"));
