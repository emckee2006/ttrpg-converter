...
//! let response = client.get("https://example.com/asset.jpg").await?;
...
impl EnhancedHttpClient {
    ...
    pub async fn with_config(config: HttpClientConfig) -> AssetResult<Self> {
        ...
        let mut client_builder = ClientBuilder::new(base_client).with(TracingMiddleware::default()); // Add tracing support
        ...
        if config.cache.enabled {
            ...
            info!(
                "HTTP caching enabled with {} mode",
    ...
    pub async fn get_enhanced(&self, url: &str) -> AssetResult<EnhancedHttpResponse> {
        ...
        info!(
            "Enhanced HTTP GET successful: {} ({}ms, cache: {})",
            url,
            response_time,
    ...
    pub async fn download_asset_with_verification(
        ...
        let enhanced_response = self.get_enhanced(url).await?;
        let content = enhanced_response.response.bytes().await.map_err(|e| {
    ...
    fn validate_url(&self, url: &str) -> AssetResult<()> {
        ...
        let parsed = Url::parse(url).map_err(|e| AssetError::ValidationError {
            asset_path: url.to_string(),
