...
//! 
//! // Cache asset data
//! cache.insert("asset_123", asset_data).await;
//! let cached_asset = cache.get("asset_123").await;
//! ```
...
impl CacheStats {
    ...
    pub fn update_access_time(&mut self, access_time_us: u64) {
        ...
        if total_accesses > 0 {
            self.average_access_time_us = 
                ((self.average_access_time_us * (total_accesses - 1) as f64) + access_time_us as f64) 
    ...
    V: Send + Sync + Clone + 'static,
{
    ...
    pub async fn new(name: String, config: CacheConfig) -> AssetResult<Self> {
        info!("Creating professional memory cache '{}' with {} max capacity", name, config.max_capacity);
        ...
        F: std::future::Future<Output = V>,
    {
        ...
        let result = self.cache.get_with(key, async { f.await }).await;
    ...
    async fn start_cleanup_task(&self) -> AssetResult<()> {
        ...
        debug!("Started cleanup task for cache '{}' with {} second interval", 
               self.name, interval);
        Ok(())
    }
}
...
impl CacheManager {
    ...
    pub async fn new() -> AssetResult<Self> {
        ...
        let url_validations = UrlValidationCache::with_default_config("url_validations".to_string()).await?;
    ...
    pub async fn with_configs(
        ...
        let validations = ValidationCache::new("validations".to_string(), validation_config).await?;
        let metadata = MetadataCache::new("metadata".to_string(), metadata_config).await?;
        let url_validations = UrlValidationCache::new("url_validations".to_string(), url_validation_config).await?;
...
mod tests {
    ...
    async fn test_memory_cache_creation() {
        let cache: MemoryCache<String, String> = MemoryCache::with_default_config(
            "test".to_string()
        ).await.unwrap();
    ...
    async fn test_cache_manager() {
        ...
        // Test asset cache
        manager.assets.insert("asset1".to_string(), vec![1, 2, 3]).await;
