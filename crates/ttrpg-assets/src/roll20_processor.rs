...
use crate::service::{AssetType, RustAssetService};
...
use ttrpg_core::{
    ...
    services::{AssetInfo, AssetService, AssetStats, LoggingService},
};
...
impl Roll20AssetProcessor {
    ...
    #[instrument(name = "roll20_processor_new", skip(base_service, logger))]
    pub fn new(
        base_service: Arc<RustAssetService>,
        config: Roll20ProcessorConfig,
        logger: Option<Arc<dyn LoggingService>>,
    ...
    pub async fn with_defaults(
        ...
        let base_service = Arc::new(RustAssetService::new(cache_dir, logger.clone()).await?);
        Ok(Self::new(base_service, Roll20ProcessorConfig::default(), logger))
    }
