...
impl IntegrationAssetService {
    fn new() -> Self {
        Self { asset_count: AtomicUsize::new(0) }
    }
...
fn test_service_manager_full_lifecycle() {
    ...
    let asset_info = assets
        .get_asset_info(&PathBuf::from("/test/path"))
        .expect("Asset info failed");
...
fn test_service_coordination_workflow() {
    ...
    let validation_result = validator
        .validate_campaign(&campaign)
        .expect("Campaign validation failed");
    ...
    let asset_result = assets
        .download_asset("https://example.com/test-asset.png", &PathBuf::from("/test/output"))
    ...
    let file_validation = validator
        .validate_file_path("/test/output/test-asset.png")
    ...
    let handles: Vec<_> = (0..10)
        .map(|i| {
            ...
            std::thread::spawn(move || {
                ...
                let _asset_info = assets
                    .get_asset_info(&PathBuf::from(&format!("/test/{i}")))
                    .unwrap();
            })
        })
        .collect();
...
fn test_service_error_handling() {
    ...
    let validation_result = validator
        ...
        .expect("JSON validation should not panic");
...
fn test_service_manager_with_defaults() {
    ...
    let manager =
        DefaultServiceManager::new().expect("Failed to create service manager with defaults");
