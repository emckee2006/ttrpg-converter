...
impl RustValidator {
    ...
    fn initialize_default_schemas(&mut self) -> ConversionResult<()> {
        ...
        self.add_entity_schema("actor", EntitySchema {
        ...
        // Item schema
        self.add_entity_schema("item", EntitySchema {
            required_fields: ["name", "type"].iter().map(|s| s.to_string()).collect(),
            optional_fields: [
        ...
        // Scene schema
        self.add_entity_schema("scene", EntitySchema {
            required_fields: ["name", "width", "height"].iter().map(|s| s.to_string()).collect(),
            optional_fields: [
                ("background_color".to_string(), serde_json::Value::String("#ffffff".to_string())),
                    ...
                    if let Some(actor_type) = data.get("type").and_then(|v| v.as_str()) {
                        if !["character", "npc", "vehicle"].contains(&actor_type) {
                            errors.push(ValidationError {
                                ...
                                message: format!("Invalid actor type: {}. Must be one of: character, npc, vehicle", actor_type),
                    ...
                    if let Some(item_type) = data.get("type").and_then(|v| v.as_str()) {
                        if !["weapon", "armor", "consumable", "tool", "loot"].contains(&item_type) {
                            errors.push(ValidationError {
                                ...
                                message: format!("Invalid item type: {}. Must be one of: weapon, armor, consumable, tool, loot", item_type),
                                entity_type: entity_type.to_string(),
                                entity_id: None,
                                field_path: Some("type".to_string()),
                ...
                "validate_scene_dimensions" => {
                    ...
                    if width <= 0.0 || height <= 0.0 {
                        errors.push(ValidationError {
                            ...
                            actual: Some(format!("width={}, height={}", width, height)),
                        });
                    }
...
impl crate::services::ValidationService for RustValidator {
    fn validate_campaign(&self, campaign: &Campaign) -> ConversionResult<crate::services::ValidationResult> {
    ...
    fn validate_required_fields(
        ...
        if let Some(schema) = self.schemas.get(entity_type) {
            let obj = data.as_object().ok_or_else(|| {
                crate::error::ConversionError::ValidationError {
                    entity_type: entity_type.to_string(),
    ...
    fn validate_entity_data(
        ...
        Ok(crate::services::ValidationResult {
            is_valid: all_issues.iter().all(|i| i.severity != crate::services::IssueSeverity::Error),
            errors: all_issues,
    ...
    fn validate_file_path(&self, path: &str) -> ConversionResult<crate::services::ValidationResult> {
        self.update_stats(|stats| {
            stats.rules_applied += 1;
            stats.fields_validated += 1;
        });
    ...
    fn validate_json_data(&self, data: &serde_json::Value) -> ConversionResult<crate::services::ValidationResult> {
        self.update_stats(|stats| {
            stats.rules_applied += 1;
            stats.fields_validated += 1;
        });
            ...
            serde_json::Value::Object(obj) => {
                ...
                if self.count_json_depth(data) > 10 {
                    warnings.push(crate::services::ValidationIssue {
                        ...
                        message: format!("JSON has very deep nesting ({} levels)", self.count_json_depth(data)),
