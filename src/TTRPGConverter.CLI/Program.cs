...
using TTRPGConverter.CLI.Services;
...
public class Program
{
    ...
    private static IServiceProvider ConfigureServices()
    {
        ...
        // Use a DbContextFactory to allow for parallel processing and long-running services
        ...
        services.AddTransient<ICompendiumManager, SqliteCompendiumManager>();
        services.AddTransient<CompendiumCacheBuilder>();
        services.AddTransient<FoundryModuleService>(); // Register the new service
        ...
        services.AddTransient<TestCampaignCommand>(); // Remove duplicate registration
    ...
    private static RootCommand BuildRootCommand(IServiceProvider serviceProvider)
    {
        ...
        var systemOpt = new Option<string>("--system") { Description = "The target Foundry VTT system (e.g., dnd5e, pf2e)"};
        
        var convertCampaignCommand = new Command("convert-campaign", "Convert a Roll20 campaign ZIP to a Foundry VTT world");
        ...
        convertCampaignCommand.SetAction(async (ParseResult parseResult) => {
        ...
        var foundryPathOpt = new Option<string>("--foundry-data-path") { Description = "Override Foundry data directory path" };

        var updateCompendiumCommand = new Command("update-compendium", "Create or update the compendium database cache");
        updateCompendiumCommand.Options.Add(outputOpt);
        updateCompendiumCommand.Options.Add(foundryPathOpt);
        
        updateCompendiumCommand.SetAction(async (ParseResult parseResult) => {
        ...
        var zipPathArg = new Argument<string>("zipPath") { Description = "Path to the Roll20 campaign ZIP file" };
        ...
        testCampaignCommand.SetAction(async (ParseResult parseResult) => {
        ...
        testCompendiumCommand.SetAction(async (ParseResult parseResult) => {
            var handler = serviceProvider.GetRequiredService<TestCompendiumCommand>();
            var module = parseResult.GetValue(moduleOpt);
            var foundryPath = parseResult.GetValue(foundryPathOptCompendium);
            var verbose = parseResult.GetValue(verboseOpt);
            await handler.ExecuteAsync(module, foundryPath, verbose);
        });
